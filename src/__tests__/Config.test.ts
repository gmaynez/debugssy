...
describe('ConfigManager', () => {
  ...
  describe('Configuration Validation', () => {
    ...
    it('should validate automation level enum values', () => {
      ...
      expect(DebugConfigurationSchema.safeParse(validAssistedConfig).success).toBe(true);
      expect(DebugConfigurationSchema.safeParse(validFullConfig).success).toBe(true);
    });

    it('should validate expression validation level enum values', () => {
      const validLevels = ['strict', 'moderate', 'permissive', 'disabled'] as const;
    ...
    it('should validate wait for breakpoint timeout range', () => {
      ...
      expect(DebugConfigurationSchema.safeParse(validTimeout).success).toBe(true);

      const tooLowTimeout = { ...validTimeout, waitForBreakpointTimeout: 500 };
      expect(DebugConfigurationSchema.safeParse(tooLowTimeout).success).toBe(false);

      const tooHighTimeout = {
        ...validTimeout,
        waitForBreakpointTimeout: 400000,
      };
      expect(DebugConfigurationSchema.safeParse(tooHighTimeout).success).toBe(false);
    });
    ...
    it('should validate max expression length range', () => {
      ...
      expect(DebugConfigurationSchema.safeParse(validLength).success).toBe(true);

      const tooLowLength = { ...validLength, maxExpressionLength: 10 };
      expect(DebugConfigurationSchema.safeParse(tooLowLength).success).toBe(false);

      const tooHighLength = { ...validLength, maxExpressionLength: 500 };
      expect(DebugConfigurationSchema.safeParse(tooHighLength).success).toBe(false);
    });
  });
  ...
  describe('Configuration Change Events', () => {
    it('should fire onConfigChange event when configuration changes', () => {
      ...
      const changeHandler = (vscode.workspace.onDidChangeConfiguration as any).mock.calls[0][0];

      // Mock that debugssy configuration changed
      const mockEvent = {
        affectsConfiguration: vi.fn((section: string) => section === 'debugssy'),
      };
