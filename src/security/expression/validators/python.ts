...
import { PYTHON_SAFE_METHODS, PYTHON_SAFE_STATIC_FUNCTIONS } from '../safeLists';

/**
 * Detects critical Python operations (os, subprocess, file operations).
 */
export function detectPythonCritical(expression: string): ValidationResult | null {
  ...
  const openNamedModeWritePattern = /\bopen\s*\([^)]*mode\s*=\s*['"][^'"]*(?:[wax]|\+)[^'"]*['"]/i;
