...
export function detectPHPCritical(expression: string): ValidationResult | null {
  ...
  if (
    /\b(exec|system|shell_exec|passthru|proc_open|popen|pcntl_exec|`)\s*[(`]/i.test(
      expression,
    )
  ) {
    return {
...
export function validatePHP(
  ...
  if (
    /\b(eval|assert|create_function|call_user_func|call_user_func_array)\s*\(/i.test(
      expression,
    )
  ) {
    return {
  ...
  if (
    /\b(include|require|include_once|require_once)\s*\(?(?!\s*['"])/i.test(
      expression,
    ) ||
