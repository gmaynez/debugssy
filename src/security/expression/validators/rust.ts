 ...
 */
export function detectRustCritical(expression: string): ValidationResult | null {
  // Process execution
  if (
  ...
  if (
    ...
    /\bOpenOptions\s*::\s*new\s*\(\s*\)\s*\.\s*(write|append|create|truncate)/i.test(expression)
  ) {
    return {
      allowed: false,
...
export function validateRust(
  ...
  if (/[\w_\]]\s*\(/.test(expression)) {
    ...
    const whitelistResult = checkAgainstWhitelists(calls, RUST_SAFE_FUNCTIONS, RUST_SAFE_FUNCTIONS);
