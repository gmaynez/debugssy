...
const DebugCrashArgsSchema = z.object({
  errorMessage: z
    .string()
    .min(1, { error: "errorMessage is required and must not be empty" }),
...
const InspectFunctionArgsSchema = z.object({
  functionName: z
    .string()
    .min(1, { error: "functionName is required and must not be empty" }),
  filePath: z
    .string()
    .min(1, { error: "filePath is required and must not be empty" }),
...
const DebugLoopArgsSchema = z.object({
  loopLocation: z
    .string()
    .min(1, { error: "loopLocation is required and must not be empty" }),
...
const AutoDebugSessionArgsSchema = z.object({
  issue: z
    .string()
    .min(1, { error: "issue is required and must not be empty" }),
...
export class PromptHandler {
        ...
        arguments: [
          {
            name: "loopLocation",
            description:
              "Description of where the loop is (file and approximate line)",
            required: true,
          },
          {
    ...
    switch (promptName) {
      ...
      case "auto-debug-session": {
        if (automationLevel !== "full") {
          throw new Error(
            "auto-debug-session prompt requires full automation mode",
          );
        }
  ...
  private generateDebugCrashPrompt(
    args: DebugCrashArgs,
  ...
  private generateTraceVariablePrompt(
    args: TraceVariableArgs,
    automationLevel: string,
  ): any {
    ...
    const expected = args.expectedValue
      ...
      : "";
    const actual = args.actualValue
      ? ` but is actually ${args.actualValue}`
      : "";
  ...
  private generateInspectFunctionPrompt(
  ...
  private generateDebugLoopPrompt(
    args: DebugLoopArgs,
    automationLevel: string,
  ): any {
