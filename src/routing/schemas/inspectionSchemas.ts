    ...
    inputSchema: {
      ...
      properties: {
        scope: {
          type: "string",
          description:
            'Optional scope prefix to filter (e.g., "Local" matches "Local: functionName", "Global" matches "Global"). Case-insensitive. Recommended to reduce verbosity.',
        },
        frameId: {
          type: "number",
    ...
    inputSchema: {
      ...
      properties: {
        expression: {
          type: "string",
          description:
            "Expression to evaluate. Simple read-only expressions (variable access, property access, arithmetic, comparisons) are allowed automatically. Complex expressions with potential side effects (function calls, assignments) require user approval via elicitation. Length is limited for security.",
        },
        frameId: {
          type: "number",
          description: "Optional frame ID (defaults to current frame)",
    ...
    inputSchema: {
      ...
      properties: {
        category: {
          type: "string",
          description:
            'Filter by output category: "console", "stdout", "stderr", "telemetry". Recommended to reduce verbosity.',
        ...
        since: {
          type: "number",
          description:
            "Unix timestamp (ms) to filter entries since this time. Only returns entries newer than this timestamp.",
        },
        clear: {
          type: "boolean",
          description:
            "If true, clears the console output buffer after reading. Default: false.",
        },
      },
    },
  },
