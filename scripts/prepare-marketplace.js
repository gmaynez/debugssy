...
function status() {
  ...
  console.log(
    `  ${README_PATH}: ${fs.existsSync(README_PATH) ? "✓ exists" : "✗ missing"}`,
  );
  console.log(
    `  ${MARKETPLACE_PATH}: ${fs.existsSync(MARKETPLACE_PATH) ? "✓ exists" : "✗ missing"}`,
  );
  console.log(
    `  ${README_BACKUP}: ${fs.existsSync(README_BACKUP) ? "⚠️ exists (needs restore)" : "✓ not found"}`,
  );
  console.log(
    `  ${MARKETPLACE_BACKUP}: ${fs.existsSync(MARKETPLACE_BACKUP) ? "⚠️ exists (needs restore)" : "✓ not found"}`,
  );
